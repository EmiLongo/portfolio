...
export const ProjectCard = ({ projectInfo }) => {
  const { title, description, details, images, links, icons } = projectInfo;
    ...
    <>
      <div className="project-item">
        <div className="project-card-header">
				<h4>{texts.projects[`projectscard${projectInfo.id}`].h4}</h4>
				<p>{texts.projects[`projectscard${projectInfo.id}`].p}</p>
        </div>
          ...
          <div className="project-imgs-header">
            {links.map(({ icon, url }, index) => (
              <img
                key={index}
                src={
                  icon === "redirect" ? redirect : icon === "github" && github
                }
            ...
            <img
              ...
              style={
                isShowInfo
                  ? { filter: "invert(100%) drop-shadow(0 0 10px #00ff6a)" }
                  : {}
              }
              alt="Info Proyecto"
              width="32"
          ...
          <div className="project-imgs-footer">
            {images.map((_, index) => (
              <img
                ...
                alt={`Vista desde ${
                  index === 0 ? "Desktop" : index === 1 ? "Tablet" : "Mobile"
                }`}
                width="32"
          ...
          {isShowInfo && (
            <div className="project-info">
              ...
              <div className="project-info-footer">
                ...
                {icons.javascript && (
                  <img src={javascript} width="32" alt="javascript" />
                )}
                {icons.react && <img src={react} width="32" alt="react" />}
                {icons.tailwind && (
                  <img src={tailwind} width="32" alt="tailwind" />
                )}
                {icons.netlify && (
                  <img src={netlify} width="32" alt="netlify" />
                )}
